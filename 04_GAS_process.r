   ...
   # }
    ...
    if(distr == "norm"){
        ...
        if(!is.null(sigma_weights)){
            ...
            cov_mat <- diag(static_param[-c(1:((gas_b_lag + gas_a_lag )*n_tv_param + length(tv_par) ))])
        }
        ...
        cov_mat <- diag(static_param[-c(1:((gas_b_lag + gas_a_lag )*n_tv_param + length(tv_par) + 1))])
    ...
    for( i in start_it:end_it){
        ...
        if(out == TRUE){
            ...
            Score_out <- cbind(Score_out, S_t_new$score)
        }
...
if( out == TRUE){
    ...
    return(list(pars = param_out, values = data_out, llh = llh_out, par_cnt = par_cnt, nu= nu, score_out = Score_out))
}else{
    return(negative_llh)
}
}
